#include <stdint.h>
#include <string.h>
#include "stm32l1xx_hal_flash.h"
#include "main.h"
#include "my_aes.h"
#include "acc_aes.h"

uint32_t aes_rkeys_global[AES_ROUNDS + 1][AES_KEY_WSIZE];
uint8_t aes_key_global[AES_KEY_SIZE] = {
  0x2b, 0x7e, 0x15, 0x16,
  0x28, 0xae, 0xd2, 0xa6,
  0xab, 0xf7, 0x15, 0x88,
  0x09, 0xcf, 0x4f, 0x3c
};
uint8_t aes_init_vec_global[AES_BLOCK_SIZE] = {
  0x00, 0x01, 0x02, 0x03,
  0x04, 0x05, 0x06, 0x07,
  0x08, 0x09, 0x0a, 0x0b,
  0x0c, 0x0d, 0x0e, 0x0f
};
uint8_t aes_state_global[AES_STATE_ELEMS] = {0};
uint8_t aes_ciphertext_buf[AES_CIPHERTEXT_BUF_SIZE];
uint8_t aes_deciphertext_buf[AES_DECIPHERTEXT_BUF_SIZE];

// Forward Sbox lookup table
const uint8_t aes_sbox[256] = {
  
};

const uint8_t aes_inv_sbox[256] = {
  
};

// Rcon lookup table
const uint32_t aes_rcon[10] = {
    0x00000001, 0x00000002, 0x00000004, 0x00000008,
    0x00000010, 0x00000020, 0x00000040, 0x00000080,
    0x0000001B, 0x00000036
};

// Multiplication in GF(2,8) by 6 different coefficients lookup table
const uint8_t aes_gf_mul[256][6] = {
   {0x00,0x00,0x00,0x00,0x00,0x00},{0x02,0x03,0x09,0x0b,0x0d,0x0e},
   {0x04,0x06,0x12,0x16,0x1a,0x1c},{0x06,0x05,0x1b,0x1d,0x17,0x12},
   {0x08,0x0c,0x24,0x2c,0x34,0x38},{0x0a,0x0f,0x2d,0x27,0x39,0x36},
   {0x0c,0x0a,0x36,0x3a,0x2e,0x24},{0x0e,0x09,0x3f,0x31,0x23,0x2a},
   {0x10,0x18,0x48,0x58,0x68,0x70},{0x12,0x1b,0x41,0x53,0x65,0x7e},
   {0x14,0x1e,0x5a,0x4e,0x72,0x6c},{0x16,0x1d,0x53,0x45,0x7f,0x62},
   {0x18,0x14,0x6c,0x74,0x5c,0x48},{0x1a,0x17,0x65,0x7f,0x51,0x46},
   {0x1c,0x12,0x7e,0x62,0x46,0x54},{0x1e,0x11,0x77,0x69,0x4b,0x5a},
   {0x20,0x30,0x90,0xb0,0xd0,0xe0},{0x22,0x33,0x99,0xbb,0xdd,0xee},
   {0x24,0x36,0x82,0xa6,0xca,0xfc},{0x26,0x35,0x8b,0xad,0xc7,0xf2},
   {0x28,0x3c,0xb4,0x9c,0xe4,0xd8},{0x2a,0x3f,0xbd,0x97,0xe9,0xd6},
   {0x2c,0x3a,0xa6,0x8a,0xfe,0xc4},{0x2e,0x39,0xaf,0x81,0xf3,0xca},
   {0x30,0x28,0xd8,0xe8,0xb8,0x90},{0x32,0x2b,0xd1,0xe3,0xb5,0x9e},
   {0x34,0x2e,0xca,0xfe,0xa2,0x8c},{0x36,0x2d,0xc3,0xf5,0xaf,0x82},
   {0x38,0x24,0xfc,0xc4,0x8c,0xa8},{0x3a,0x27,0xf5,0xcf,0x81,0xa6},
   {0x3c,0x22,0xee,0xd2,0x96,0xb4},{0x3e,0x21,0xe7,0xd9,0x9b,0xba},
   {0x40,0x60,0x3b,0x7b,0xbb,0xdb},{0x42,0x63,0x32,0x70,0xb6,0xd5},
   {0x44,0x66,0x29,0x6d,0xa1,0xc7},{0x46,0x65,0x20,0x66,0xac,0xc9},
   {0x48,0x6c,0x1f,0x57,0x8f,0xe3},{0x4a,0x6f,0x16,0x5c,0x82,0xed},
   {0x4c,0x6a,0x0d,0x41,0x95,0xff},{0x4e,0x69,0x04,0x4a,0x98,0xf1},
   {0x50,0x78,0x73,0x23,0xd3,0xab},{0x52,0x7b,0x7a,0x28,0xde,0xa5},
   {0x54,0x7e,0x61,0x35,0xc9,0xb7},{0x56,0x7d,0x68,0x3e,0xc4,0xb9},
   {0x58,0x74,0x57,0x0f,0xe7,0x93},{0x5a,0x77,0x5e,0x04,0xea,0x9d},
   {0x5c,0x72,0x45,0x19,0xfd,0x8f},{0x5e,0x71,0x4c,0x12,0xf0,0x81},
   {0x60,0x50,0xab,0xcb,0x6b,0x3b},{0x62,0x53,0xa2,0xc0,0x66,0x35},
   {0x64,0x56,0xb9,0xdd,0x71,0x27},{0x66,0x55,0xb0,0xd6,0x7c,0x29},
   {0x68,0x5c,0x8f,0xe7,0x5f,0x03},{0x6a,0x5f,0x86,0xec,0x52,0x0d},
   {0x6c,0x5a,0x9d,0xf1,0x45,0x1f},{0x6e,0x59,0x94,0xfa,0x48,0x11},
   {0x70,0x48,0xe3,0x93,0x03,0x4b},{0x72,0x4b,0xea,0x98,0x0e,0x45},
   {0x74,0x4e,0xf1,0x85,0x19,0x57},{0x76,0x4d,0xf8,0x8e,0x14,0x59},
   {0x78,0x44,0xc7,0xbf,0x37,0x73},{0x7a,0x47,0xce,0xb4,0x3a,0x7d},
   {0x7c,0x42,0xd5,0xa9,0x2d,0x6f},{0x7e,0x41,0xdc,0xa2,0x20,0x61},
   {0x80,0xc0,0x76,0xf6,0x6d,0xad},{0x82,0xc3,0x7f,0xfd,0x60,0xa3},
   {0x84,0xc6,0x64,0xe0,0x77,0xb1},{0x86,0xc5,0x6d,0xeb,0x7a,0xbf},
   {0x88,0xcc,0x52,0xda,0x59,0x95},{0x8a,0xcf,0x5b,0xd1,0x54,0x9b},
   {0x8c,0xca,0x40,0xcc,0x43,0x89},{0x8e,0xc9,0x49,0xc7,0x4e,0x87},
   {0x90,0xd8,0x3e,0xae,0x05,0xdd},{0x92,0xdb,0x37,0xa5,0x08,0xd3},
   {0x94,0xde,0x2c,0xb8,0x1f,0xc1},{0x96,0xdd,0x25,0xb3,0x12,0xcf},
   {0x98,0xd4,0x1a,0x82,0x31,0xe5},{0x9a,0xd7,0x13,0x89,0x3c,0xeb},
   {0x9c,0xd2,0x08,0x94,0x2b,0xf9},{0x9e,0xd1,0x01,0x9f,0x26,0xf7},
   {0xa0,0xf0,0xe6,0x46,0xbd,0x4d},{0xa2,0xf3,0xef,0x4d,0xb0,0x43},
   {0xa4,0xf6,0xf4,0x50,0xa7,0x51},{0xa6,0xf5,0xfd,0x5b,0xaa,0x5f},
   {0xa8,0xfc,0xc2,0x6a,0x89,0x75},{0xaa,0xff,0xcb,0x61,0x84,0x7b},
   {0xac,0xfa,0xd0,0x7c,0x93,0x69},{0xae,0xf9,0xd9,0x77,0x9e,0x67},
   {0xb0,0xe8,0xae,0x1e,0xd5,0x3d},{0xb2,0xeb,0xa7,0x15,0xd8,0x33},
   {0xb4,0xee,0xbc,0x08,0xcf,0x21},{0xb6,0xed,0xb5,0x03,0xc2,0x2f},
   {0xb8,0xe4,0x8a,0x32,0xe1,0x05},{0xba,0xe7,0x83,0x39,0xec,0x0b},
   {0xbc,0xe2,0x98,0x24,0xfb,0x19},{0xbe,0xe1,0x91,0x2f,0xf6,0x17},
   {0xc0,0xa0,0x4d,0x8d,0xd6,0x76},{0xc2,0xa3,0x44,0x86,0xdb,0x78},
   {0xc4,0xa6,0x5f,0x9b,0xcc,0x6a},{0xc6,0xa5,0x56,0x90,0xc1,0x64},
   {0xc8,0xac,0x69,0xa1,0xe2,0x4e},{0xca,0xaf,0x60,0xaa,0xef,0x40},
   {0xcc,0xaa,0x7b,0xb7,0xf8,0x52},{0xce,0xa9,0x72,0xbc,0xf5,0x5c},
   {0xd0,0xb8,0x05,0xd5,0xbe,0x06},{0xd2,0xbb,0x0c,0xde,0xb3,0x08},
   {0xd4,0xbe,0x17,0xc3,0xa4,0x1a},{0xd6,0xbd,0x1e,0xc8,0xa9,0x14},
   {0xd8,0xb4,0x21,0xf9,0x8a,0x3e},{0xda,0xb7,0x28,0xf2,0x87,0x30},
   {0xdc,0xb2,0x33,0xef,0x90,0x22},{0xde,0xb1,0x3a,0xe4,0x9d,0x2c},
   {0xe0,0x90,0xdd,0x3d,0x06,0x96},{0xe2,0x93,0xd4,0x36,0x0b,0x98},
   {0xe4,0x96,0xcf,0x2b,0x1c,0x8a},{0xe6,0x95,0xc6,0x20,0x11,0x84},
   {0xe8,0x9c,0xf9,0x11,0x32,0xae},{0xea,0x9f,0xf0,0x1a,0x3f,0xa0},
   {0xec,0x9a,0xeb,0x07,0x28,0xb2},{0xee,0x99,0xe2,0x0c,0x25,0xbc},
   {0xf0,0x88,0x95,0x65,0x6e,0xe6},{0xf2,0x8b,0x9c,0x6e,0x63,0xe8},
   {0xf4,0x8e,0x87,0x73,0x74,0xfa},{0xf6,0x8d,0x8e,0x78,0x79,0xf4},
   {0xf8,0x84,0xb1,0x49,0x5a,0xde},{0xfa,0x87,0xb8,0x42,0x57,0xd0},
   {0xfc,0x82,0xa3,0x5f,0x40,0xc2},{0xfe,0x81,0xaa,0x54,0x4d,0xcc},
   {0x1b,0x9b,0xec,0xf7,0xda,0x41},{0x19,0x98,0xe5,0xfc,0xd7,0x4f},
   {0x1f,0x9d,0xfe,0xe1,0xc0,0x5d},{0x1d,0x9e,0xf7,0xea,0xcd,0x53},
   {0x13,0x97,0xc8,0xdb,0xee,0x79},{0x11,0x94,0xc1,0xd0,0xe3,0x77},
   {0x17,0x91,0xda,0xcd,0xf4,0x65},{0x15,0x92,0xd3,0xc6,0xf9,0x6b},
   {0x0b,0x83,0xa4,0xaf,0xb2,0x31},{0x09,0x80,0xad,0xa4,0xbf,0x3f},
   {0x0f,0x85,0xb6,0xb9,0xa8,0x2d},{0x0d,0x86,0xbf,0xb2,0xa5,0x23},
   {0x03,0x8f,0x80,0x83,0x86,0x09},{0x01,0x8c,0x89,0x88,0x8b,0x07},
   {0x07,0x89,0x92,0x95,0x9c,0x15},{0x05,0x8a,0x9b,0x9e,0x91,0x1b},
   {0x3b,0xab,0x7c,0x47,0x0a,0xa1},{0x39,0xa8,0x75,0x4c,0x07,0xaf},
   {0x3f,0xad,0x6e,0x51,0x10,0xbd},{0x3d,0xae,0x67,0x5a,0x1d,0xb3},
   {0x33,0xa7,0x58,0x6b,0x3e,0x99},{0x31,0xa4,0x51,0x60,0x33,0x97},
   {0x37,0xa1,0x4a,0x7d,0x24,0x85},{0x35,0xa2,0x43,0x76,0x29,0x8b},
   {0x2b,0xb3,0x34,0x1f,0x62,0xd1},{0x29,0xb0,0x3d,0x14,0x6f,0xdf},
   {0x2f,0xb5,0x26,0x09,0x78,0xcd},{0x2d,0xb6,0x2f,0x02,0x75,0xc3},
   {0x23,0xbf,0x10,0x33,0x56,0xe9},{0x21,0xbc,0x19,0x38,0x5b,0xe7},
   {0x27,0xb9,0x02,0x25,0x4c,0xf5},{0x25,0xba,0x0b,0x2e,0x41,0xfb},
   {0x5b,0xfb,0xd7,0x8c,0x61,0x9a},{0x59,0xf8,0xde,0x87,0x6c,0x94},
   {0x5f,0xfd,0xc5,0x9a,0x7b,0x86},{0x5d,0xfe,0xcc,0x91,0x76,0x88},
   {0x53,0xf7,0xf3,0xa0,0x55,0xa2},{0x51,0xf4,0xfa,0xab,0x58,0xac},
   {0x57,0xf1,0xe1,0xb6,0x4f,0xbe},{0x55,0xf2,0xe8,0xbd,0x42,0xb0},
   {0x4b,0xe3,0x9f,0xd4,0x09,0xea},{0x49,0xe0,0x96,0xdf,0x04,0xe4},
   {0x4f,0xe5,0x8d,0xc2,0x13,0xf6},{0x4d,0xe6,0x84,0xc9,0x1e,0xf8},
   {0x43,0xef,0xbb,0xf8,0x3d,0xd2},{0x41,0xec,0xb2,0xf3,0x30,0xdc},
   {0x47,0xe9,0xa9,0xee,0x27,0xce},{0x45,0xea,0xa0,0xe5,0x2a,0xc0},
   {0x7b,0xcb,0x47,0x3c,0xb1,0x7a},{0x79,0xc8,0x4e,0x37,0xbc,0x74},
   {0x7f,0xcd,0x55,0x2a,0xab,0x66},{0x7d,0xce,0x5c,0x21,0xa6,0x68},
   {0x73,0xc7,0x63,0x10,0x85,0x42},{0x71,0xc4,0x6a,0x1b,0x88,0x4c},
   {0x77,0xc1,0x71,0x06,0x9f,0x5e},{0x75,0xc2,0x78,0x0d,0x92,0x50},
   {0x6b,0xd3,0x0f,0x64,0xd9,0x0a},{0x69,0xd0,0x06,0x6f,0xd4,0x04},
   {0x6f,0xd5,0x1d,0x72,0xc3,0x16},{0x6d,0xd6,0x14,0x79,0xce,0x18},
   {0x63,0xdf,0x2b,0x48,0xed,0x32},{0x61,0xdc,0x22,0x43,0xe0,0x3c},
   {0x67,0xd9,0x39,0x5e,0xf7,0x2e},{0x65,0xda,0x30,0x55,0xfa,0x20},
   {0x9b,0x5b,0x9a,0x01,0xb7,0xec},{0x99,0x58,0x93,0x0a,0xba,0xe2},
   {0x9f,0x5d,0x88,0x17,0xad,0xf0},{0x9d,0x5e,0x81,0x1c,0xa0,0xfe},
   {0x93,0x57,0xbe,0x2d,0x83,0xd4},{0x91,0x54,0xb7,0x26,0x8e,0xda},
   {0x97,0x51,0xac,0x3b,0x99,0xc8},{0x95,0x52,0xa5,0x30,0x94,0xc6},
   {0x8b,0x43,0xd2,0x59,0xdf,0x9c},{0x89,0x40,0xdb,0x52,0xd2,0x92},
   {0x8f,0x45,0xc0,0x4f,0xc5,0x80},{0x8d,0x46,0xc9,0x44,0xc8,0x8e},
   {0x83,0x4f,0xf6,0x75,0xeb,0xa4},{0x81,0x4c,0xff,0x7e,0xe6,0xaa},
   {0x87,0x49,0xe4,0x63,0xf1,0xb8},{0x85,0x4a,0xed,0x68,0xfc,0xb6},
   {0xbb,0x6b,0x0a,0xb1,0x67,0x0c},{0xb9,0x68,0x03,0xba,0x6a,0x02},
   {0xbf,0x6d,0x18,0xa7,0x7d,0x10},{0xbd,0x6e,0x11,0xac,0x70,0x1e},
   {0xb3,0x67,0x2e,0x9d,0x53,0x34},{0xb1,0x64,0x27,0x96,0x5e,0x3a},
   {0xb7,0x61,0x3c,0x8b,0x49,0x28},{0xb5,0x62,0x35,0x80,0x44,0x26},
   {0xab,0x73,0x42,0xe9,0x0f,0x7c},{0xa9,0x70,0x4b,0xe2,0x02,0x72},
   {0xaf,0x75,0x50,0xff,0x15,0x60},{0xad,0x76,0x59,0xf4,0x18,0x6e},
   {0xa3,0x7f,0x66,0xc5,0x3b,0x44},{0xa1,0x7c,0x6f,0xce,0x36,0x4a},
   {0xa7,0x79,0x74,0xd3,0x21,0x58},{0xa5,0x7a,0x7d,0xd8,0x2c,0x56},
   {0xdb,0x3b,0xa1,0x7a,0x0c,0x37},{0xd9,0x38,0xa8,0x71,0x01,0x39},
   {0xdf,0x3d,0xb3,0x6c,0x16,0x2b},{0xdd,0x3e,0xba,0x67,0x1b,0x25},
   {0xd3,0x37,0x85,0x56,0x38,0x0f},{0xd1,0x34,0x8c,0x5d,0x35,0x01},
   {0xd7,0x31,0x97,0x40,0x22,0x13},{0xd5,0x32,0x9e,0x4b,0x2f,0x1d},
   {0xcb,0x23,0xe9,0x22,0x64,0x47},{0xc9,0x20,0xe0,0x29,0x69,0x49},
   {0xcf,0x25,0xfb,0x34,0x7e,0x5b},{0xcd,0x26,0xf2,0x3f,0x73,0x55},
   {0xc3,0x2f,0xcd,0x0e,0x50,0x7f},{0xc1,0x2c,0xc4,0x05,0x5d,0x71},
   {0xc7,0x29,0xdf,0x18,0x4a,0x63},{0xc5,0x2a,0xd6,0x13,0x47,0x6d},
   {0xfb,0x0b,0x31,0xca,0xdc,0xd7},{0xf9,0x08,0x38,0xc1,0xd1,0xd9},
   {0xff,0x0d,0x23,0xdc,0xc6,0xcb},{0xfd,0x0e,0x2a,0xd7,0xcb,0xc5},
   {0xf3,0x07,0x15,0xe6,0xe8,0xef},{0xf1,0x04,0x1c,0xed,0xe5,0xe1},
   {0xf7,0x01,0x07,0xf0,0xf2,0xf3},{0xf5,0x02,0x0e,0xfb,0xff,0xfd},
   {0xeb,0x13,0x79,0x92,0xb4,0xa7},{0xe9,0x10,0x70,0x99,0xb9,0xa9},
   {0xef,0x15,0x6b,0x84,0xae,0xbb},{0xed,0x16,0x62,0x8f,0xa3,0xb5},
   {0xe3,0x1f,0x5d,0xbe,0x80,0x9f},{0xe1,0x1c,0x54,0xb5,0x8d,0x91},
   {0xe7,0x19,0x4f,0xa8,0x9a,0x83},{0xe5,0x1a,0x46,0xa3,0x97,0x8d}
};

static inline void aes128_add_roundkey(AES_state state, AES_rkey rk) {
  uint32_t* s = (uint32_t*) state;
  
  s[0] ^= rk[0];
  s[1] ^= rk[1];
  s[2] ^= rk[2];
  s[3] ^= rk[3];
}

static inline void aes128_sub_bytes(AES_state state) {
  state[0][0] = aes_sbox[state[0][0]];
  state[0][1] = aes_sbox[state[0][1]];
  state[0][2] = aes_sbox[state[0][2]];
  state[0][3] = aes_sbox[state[0][3]];
  state[1][0] = aes_sbox[state[1][0]];
  state[1][1] = aes_sbox[state[1][1]];
  state[1][2] = aes_sbox[state[1][2]];
  state[1][3] = aes_sbox[state[1][3]];
  state[2][0] = aes_sbox[state[2][0]];
  state[2][1] = aes_sbox[state[2][1]];
  state[2][2] = aes_sbox[state[2][2]];
  state[2][3] = aes_sbox[state[2][3]];
  state[3][0] = aes_sbox[state[3][0]];
  state[3][1] = aes_sbox[state[3][1]];
  state[3][2] = aes_sbox[state[3][2]];
  state[3][3] = aes_sbox[state[3][3]];
}

static inline void aes128_inv_sub_bytes(AES_state state) {
  state[0][0] = aes_inv_sbox[state[0][0]];
  state[0][1] = aes_inv_sbox[state[0][1]];
  state[0][2] = aes_inv_sbox[state[0][2]];
  state[0][3] = aes_inv_sbox[state[0][3]];
  state[1][0] = aes_inv_sbox[state[1][0]];
  state[1][1] = aes_inv_sbox[state[1][1]];
  state[1][2] = aes_inv_sbox[state[1][2]];
  state[1][3] = aes_inv_sbox[state[1][3]];
  state[2][0] = aes_inv_sbox[state[2][0]];
  state[2][1] = aes_inv_sbox[state[2][1]];
  state[2][2] = aes_inv_sbox[state[2][2]];
  state[2][3] = aes_inv_sbox[state[2][3]];
  state[3][0] = aes_inv_sbox[state[3][0]];
  state[3][1] = aes_inv_sbox[state[3][1]];
  state[3][2] = aes_inv_sbox[state[3][2]];
  state[3][3] = aes_inv_sbox[state[3][3]];
}

static inline void aes128_shift_rows(AES_state state) {
  uint8_t t;
  
  // shift row 1 left by 1
  t = state[0][1];
  state[0][1] = state[1][1];
  state[1][1] = state[2][1];
  state[2][1] = state[3][1];
  state[3][1] = t;
  
  // shift row 2 left by 2
  t = state[0][2];
  state[0][2] = state[2][2];
  state[2][2] = t;
  t = state[1][2];
  state[1][2] = state[3][2];
  state[3][2] = t;
  
  // shift row 3 left by 3
  t = state[3][3];
  state[3][3] = state[2][3];
  state[2][3] = state[1][3];
  state[1][3] = state[0][3];
  state[0][3] = t;
}

static inline void aes128_inv_shift_rows(AES_state state) {
  uint8_t t;
  
  // shift row 1 right by 1
  t = state[3][1];
  state[3][1] = state[2][1];
  state[2][1] = state[1][1];
  state[1][1] = state[0][1];
  state[0][1] = t;
  
  // shift row 2 right by 2
  t = state[3][2];
  state[3][2] = state[1][2];
  state[1][2] = t;
  t = state[2][2];
  state[2][2] = state[0][2];
  state[0][2] = t;
  
  // shift row 3 right by 3
  t = state[0][3];
  state[0][3] = state[1][3];
  state[1][3] = state[2][3];
  state[2][3] = state[3][3];
  state[3][3] = t;
}

static inline void aes128_mix_columns(AES_state state)
{
  // original column must be saved, calculations performed in-place
  uint8_t col[4];
  
  // column 1
  col[0] = state[0][0];
  col[1] = state[0][1];
  col[2] = state[0][2];
  col[3] = state[0][3];
  state[0][0] = aes_gf_mul[col[0]][0];
  state[0][0] ^= aes_gf_mul[col[1]][1];
  state[0][0] ^= col[2];
  state[0][0] ^= col[3];
  state[0][1] = col[0];
  state[0][1] ^= aes_gf_mul[col[1]][0];
  state[0][1] ^= aes_gf_mul[col[2]][1];
  state[0][1] ^= col[3];
  state[0][2] = col[0];
  state[0][2] ^= col[1];
  state[0][2] ^= aes_gf_mul[col[2]][0];
  state[0][2] ^= aes_gf_mul[col[3]][1];
  state[0][3] = aes_gf_mul[col[0]][1];
  state[0][3] ^= col[1];
  state[0][3] ^= col[2];
  state[0][3] ^= aes_gf_mul[col[3]][0];
  
  // column 2
  col[0] = state[1][0];
  col[1] = state[1][1];
  col[2] = state[1][2];
  col[3] = state[1][3];
  state[1][0] = aes_gf_mul[col[0]][0];
  state[1][0] ^= aes_gf_mul[col[1]][1];
  state[1][0] ^= col[2];
  state[1][0] ^= col[3];
  state[1][1] = col[0];
  state[1][1] ^= aes_gf_mul[col[1]][0];
  state[1][1] ^= aes_gf_mul[col[2]][1];
  state[1][1] ^= col[3];
  state[1][2] = col[0];
  state[1][2] ^= col[1];
  state[1][2] ^= aes_gf_mul[col[2]][0];
  state[1][2] ^= aes_gf_mul[col[3]][1];
  state[1][3] = aes_gf_mul[col[0]][1];
  state[1][3] ^= col[1];
  state[1][3] ^= col[2];
  state[1][3] ^= aes_gf_mul[col[3]][0];
  
  // column 3
  col[0] = state[2][0];
  col[1] = state[2][1];
  col[2] = state[2][2];
  col[3] = state[2][3];
  state[2][0] = aes_gf_mul[col[0]][0];
  state[2][0] ^= aes_gf_mul[col[1]][1];
  state[2][0] ^= col[2];
  state[2][0] ^= col[3];
  state[2][1] = col[0];
  state[2][1] ^= aes_gf_mul[col[1]][0];
  state[2][1] ^= aes_gf_mul[col[2]][1];
  state[2][1] ^= col[3];
  state[2][2] = col[0];
  state[2][2] ^= col[1];
  state[2][2] ^= aes_gf_mul[col[2]][0];
  state[2][2] ^= aes_gf_mul[col[3]][1];
  state[2][3] = aes_gf_mul[col[0]][1];
  state[2][3] ^= col[1];
  state[2][3] ^= col[2];
  state[2][3] ^= aes_gf_mul[col[3]][0];
  
  // column 4
  col[0] = state[3][0];
  col[1] = state[3][1];
  col[2] = state[3][2];
  col[3] = state[3][3];
  state[3][0] = aes_gf_mul[col[0]][0];
  state[3][0] ^= aes_gf_mul[col[1]][1];
  state[3][0] ^= col[2];
  state[3][0] ^= col[3];
  state[3][1] = col[0];
  state[3][1] ^= aes_gf_mul[col[1]][0];
  state[3][1] ^= aes_gf_mul[col[2]][1];
  state[3][1] ^= col[3];
  state[3][2] = col[0];
  state[3][2] ^= col[1];
  state[3][2] ^= aes_gf_mul[col[2]][0];
  state[3][2] ^= aes_gf_mul[col[3]][1];
  state[3][3] = aes_gf_mul[col[0]][1];
  state[3][3] ^= col[1];
  state[3][3] ^= col[2];
  state[3][3] ^= aes_gf_mul[col[3]][0];
}

static inline void aes128_inv_mix_columns(AES_state state)
{
  // original column must be saved, calculations performed in-place
  uint8_t col[4];
  
  // column 1
  col[0] = state[0][0];
  col[1] = state[0][1];
  col[2] = state[0][2];
  col[3] = state[0][3];
  state[0][0] = aes_gf_mul[col[0]][5];
  state[0][0] ^= aes_gf_mul[col[1]][3];
  state[0][0] ^= aes_gf_mul[col[2]][4];
  state[0][0] ^= aes_gf_mul[col[3]][2];
  state[0][1] = aes_gf_mul[col[0]][2];
  state[0][1] ^= aes_gf_mul[col[1]][5];
  state[0][1] ^= aes_gf_mul[col[2]][3];
  state[0][1] ^= aes_gf_mul[col[3]][4];
  state[0][2] = aes_gf_mul[col[0]][4];
  state[0][2] ^= aes_gf_mul[col[1]][2];
  state[0][2] ^= aes_gf_mul[col[2]][5];
  state[0][2] ^= aes_gf_mul[col[3]][3];
  state[0][3] = aes_gf_mul[col[0]][3];
  state[0][3] ^= aes_gf_mul[col[1]][4];
  state[0][3] ^= aes_gf_mul[col[2]][2];
  state[0][3] ^= aes_gf_mul[col[3]][5];

  // column 2
  col[0] = state[1][0];
  col[1] = state[1][1];
  col[2] = state[1][2];
  col[3] = state[1][3];
  state[1][0] = aes_gf_mul[col[0]][5];
  state[1][0] ^= aes_gf_mul[col[1]][3];
  state[1][0] ^= aes_gf_mul[col[2]][4];
  state[1][0] ^= aes_gf_mul[col[3]][2];
  state[1][1] = aes_gf_mul[col[0]][2];
  state[1][1] ^= aes_gf_mul[col[1]][5];
  state[1][1] ^= aes_gf_mul[col[2]][3];
  state[1][1] ^= aes_gf_mul[col[3]][4];
  state[1][2] = aes_gf_mul[col[0]][4];
  state[1][2] ^= aes_gf_mul[col[1]][2];
  state[1][2] ^= aes_gf_mul[col[2]][5];
  state[1][2] ^= aes_gf_mul[col[3]][3];
  state[1][3] = aes_gf_mul[col[0]][3];
  state[1][3] ^= aes_gf_mul[col[1]][4];
  state[1][3] ^= aes_gf_mul[col[2]][2];
  state[1][3] ^= aes_gf_mul[col[3]][5];
  
  // column 3
  col[0] = state[2][0];
  col[1] = state[2][1];
  col[2] = state[2][2];
  col[3] = state[2][3];
  state[2][0] = aes_gf_mul[col[0]][5];
  state[2][0] ^= aes_gf_mul[col[1]][3];
  state[2][0] ^= aes_gf_mul[col[2]][4];
  state[2][0] ^= aes_gf_mul[col[3]][2];
  state[2][1] = aes_gf_mul[col[0]][2];
  state[2][1] ^= aes_gf_mul[col[1]][5];
  state[2][1] ^= aes_gf_mul[col[2]][3];
  state[2][1] ^= aes_gf_mul[col[3]][4];
  state[2][2] = aes_gf_mul[col[0]][4];
  state[2][2] ^= aes_gf_mul[col[1]][2];
  state[2][2] ^= aes_gf_mul[col[2]][5];
  state[2][2] ^= aes_gf_mul[col[3]][3];
  state[2][3] = aes_gf_mul[col[0]][3];
  state[2][3] ^= aes_gf_mul[col[1]][4];
  state[2][3] ^= aes_gf_mul[col[2]][2];
  state[2][3] ^= aes_gf_mul[col[3]][5];
  
  // column 4
  col[0] = state[3][0];
  col[1] = state[3][1];
  col[2] = state[3][2];
  col[3] = state[3][3];
  state[3][0] = aes_gf_mul[col[0]][5];
  state[3][0] ^= aes_gf_mul[col[1]][3];
  state[3][0] ^= aes_gf_mul[col[2]][4];
  state[3][0] ^= aes_gf_mul[col[3]][2];
  state[3][1] = aes_gf_mul[col[0]][2];
  state[3][1] ^= aes_gf_mul[col[1]][5];
  state[3][1] ^= aes_gf_mul[col[2]][3];
  state[3][1] ^= aes_gf_mul[col[3]][4];
  state[3][2] = aes_gf_mul[col[0]][4];
  state[3][2] ^= aes_gf_mul[col[1]][2];
  state[3][2] ^= aes_gf_mul[col[2]][5];
  state[3][2] ^= aes_gf_mul[col[3]][3];
  state[3][3] = aes_gf_mul[col[0]][3];
  state[3][3] ^= aes_gf_mul[col[1]][4];
  state[3][3] ^= aes_gf_mul[col[2]][2];
  state[3][3] ^= aes_gf_mul[col[3]][5];
}

static inline void aes128_enc_block(AES_state state, AES_rkeys rkeys) {
  /*  Round0  */ aes128_add_roundkey(state, rkeys[0]);
  /*  Round1  */ aes128_sub_bytes(state); aes128_shift_rows(state); aes128_mix_columns(state); aes128_add_roundkey(state, rkeys[1]);
  /*  Round2  */ aes128_sub_bytes(state); aes128_shift_rows(state); aes128_mix_columns(state); aes128_add_roundkey(state, rkeys[2]);
  /*  Round3  */ aes128_sub_bytes(state); aes128_shift_rows(state); aes128_mix_columns(state); aes128_add_roundkey(state, rkeys[3]);
  /*  Round4  */ aes128_sub_bytes(state); aes128_shift_rows(state); aes128_mix_columns(state); aes128_add_roundkey(state, rkeys[4]);
  /*  Round5  */ aes128_sub_bytes(state); aes128_shift_rows(state); aes128_mix_columns(state); aes128_add_roundkey(state, rkeys[5]);
  /*  Round6  */ aes128_sub_bytes(state); aes128_shift_rows(state); aes128_mix_columns(state); aes128_add_roundkey(state, rkeys[6]);
  /*  Round7  */ aes128_sub_bytes(state); aes128_shift_rows(state); aes128_mix_columns(state); aes128_add_roundkey(state, rkeys[7]);
  /*  Round8  */ aes128_sub_bytes(state); aes128_shift_rows(state); aes128_mix_columns(state); aes128_add_roundkey(state, rkeys[8]);
  /*  Round9  */ aes128_sub_bytes(state); aes128_shift_rows(state); aes128_mix_columns(state); aes128_add_roundkey(state, rkeys[9]);
  /*  Round10 */ aes128_sub_bytes(state); aes128_shift_rows(state); aes128_add_roundkey(state, rkeys[10]);
}

static inline void aes128_dec_block(AES_state state, AES_rkeys rkeys) {
  /*  InvRound10  */ aes128_add_roundkey(state, rkeys[10]); aes128_inv_shift_rows(state); aes128_inv_sub_bytes(state);
  /*  InvRound9   */ aes128_add_roundkey(state, rkeys[9]); aes128_inv_mix_columns(state); aes128_inv_shift_rows(state); aes128_inv_sub_bytes(state);
  /*  InvRound8   */ aes128_add_roundkey(state, rkeys[8]); aes128_inv_mix_columns(state); aes128_inv_shift_rows(state); aes128_inv_sub_bytes(state);
  /*  InvRound7   */ aes128_add_roundkey(state, rkeys[7]); aes128_inv_mix_columns(state); aes128_inv_shift_rows(state); aes128_inv_sub_bytes(state);
  /*  InvRound6   */ aes128_add_roundkey(state, rkeys[6]); aes128_inv_mix_columns(state); aes128_inv_shift_rows(state); aes128_inv_sub_bytes(state);
  /*  InvRound5   */ aes128_add_roundkey(state, rkeys[5]); aes128_inv_mix_columns(state); aes128_inv_shift_rows(state); aes128_inv_sub_bytes(state);
  /*  InvRound4   */ aes128_add_roundkey(state, rkeys[4]); aes128_inv_mix_columns(state); aes128_inv_shift_rows(state); aes128_inv_sub_bytes(state);
  /*  InvRound3   */ aes128_add_roundkey(state, rkeys[3]); aes128_inv_mix_columns(state); aes128_inv_shift_rows(state); aes128_inv_sub_bytes(state);
  /*  InvRound2   */ aes128_add_roundkey(state, rkeys[2]); aes128_inv_mix_columns(state); aes128_inv_shift_rows(state); aes128_inv_sub_bytes(state);
  /*  InvRound1   */ aes128_add_roundkey(state, rkeys[1]); aes128_inv_mix_columns(state); aes128_inv_shift_rows(state); aes128_inv_sub_bytes(state);
  /*  InvRound0   */ aes128_add_roundkey(state, rkeys[0]);
}

void aes128_expand_key(uint8_t* key, AES_rkeys rkeys) {
  // treat rkeys as 1-dim vector
  uint32_t* rk = (uint32_t*)rkeys;
  
  // Rkey0 is cipher key
  uint32_t* keyp = (uint32_t*)key;
  rk[0] = keyp[0];
  rk[1] = keyp[1];
  rk[2] = keyp[2];
  rk[3] = keyp[3];
  
  // compute round keys
  uint8_t t;
  uint32_t w;
  uint8_t *wp;
  for(int i = 0; i < AES_ROUNDS; ++i, rk += 4) {
    w = rkeys[i][3];
    wp = (uint8_t*)&w;
    t = wp[0];
    wp[0] = aes_sbox[wp[1]];
    wp[1] = aes_sbox[wp[2]];
    wp[2] = aes_sbox[wp[3]];
    wp[3] = aes_sbox[t];
    
    rk[4] = rk[0] ^ aes_rcon[i] ^ w;
    rk[5] = rk[1] ^ rk[4];
    rk[6] = rk[2] ^ rk[5];
    rk[7] = rk[3] ^ rk[6];
  }
}

void aes128_enc_ecb(AES_rkeys rkeys, uint8_t* data, size_t size, uint8_t* buf, uint32_t* out_addr, AES_memtype memtype) {
  AES_ARG_CHECK(size);
  
  AES_state state = (AES_state)buf;
  uint32_t* wstate = (uint32_t*)state;
  for (uint8_t* p = data; size > 0; p += AES_BLOCK_SIZE, size -= AES_BLOCK_SIZE, out_addr += AES_BLOCK_WSIZE) {
    // copy block & encrypt in place
    memcpy(state, p, AES_BLOCK_SIZE);
    aes128_enc_block(state, rkeys);
    
    AES_SAVE_STATE(out_addr, wstate, memtype);
  }
}

void aes128_dec_ecb(AES_rkeys rkeys, uint8_t* data, size_t size, uint8_t* buf, uint32_t* out_addr, AES_memtype memtype) {
  AES_ARG_CHECK(size);
  
  AES_state state = (AES_state)buf;
  uint32_t* wstate = (uint32_t*)state;
  for (uint8_t* p = data; size > 0; p += AES_BLOCK_SIZE, size -= AES_BLOCK_SIZE, out_addr += AES_BLOCK_WSIZE) {
    // copy block & encrypt in place
    memcpy(state, p, AES_BLOCK_SIZE);
    aes128_dec_block(state, rkeys);
    
    AES_SAVE_STATE(out_addr, wstate, memtype);
  }
}

void aes128_enc_cbc(AES_rkeys rkeys, uint8_t* iv, uint8_t* data, size_t size, uint8_t* buf, uint32_t* out_addr, AES_memtype memtype) {
  AES_ARG_CHECK(size);
  
  AES_state state = (AES_state)buf;
  uint32_t* wstate = (uint32_t*)state;
  memcpy(state, data, AES_BLOCK_SIZE);
  
  // XOR 1st block with init vector
  uint32_t* wiv = (uint32_t*)iv;

  wstate[0] ^= wiv[0];
  wstate[1] ^= wiv[1];
  wstate[2] ^= wiv[2];
  wstate[3] ^= wiv[3];
  
  aes128_enc_block(state, rkeys);
  
  AES_SAVE_STATE(out_addr, wstate, memtype);
  size -= AES_BLOCK_SIZE;
  
  for(uint32_t* wp = (uint32_t*)data; size > 0; wp += AES_BLOCK_WSIZE, size -= AES_BLOCK_SIZE) {
    // copy block & encrypt in place
    memcpy(state, wp + 4, AES_BLOCK_SIZE);
    
    // XOR with previously encrypted block
    wstate[0] ^= out_addr[0];
    wstate[1] ^= out_addr[1];
    wstate[2] ^= out_addr[2];
    wstate[3] ^= out_addr[3];
    
    aes128_enc_block(state, rkeys);
    
    out_addr += AES_BLOCK_WSIZE;
    AES_SAVE_STATE(out_addr, wstate, memtype);
  }
}

void aes128_dec_cbc(AES_rkeys rkeys, uint8_t* iv, uint8_t* data, size_t size, uint8_t* buf, uint32_t* out_addr, AES_memtype memtype) {
  AES_ARG_CHECK(size);
  
  out_addr += size / 4 - AES_BLOCK_WSIZE; 
  AES_state state = (AES_state)buf;
  uint32_t* wstate = (uint32_t*)state;
  size -= AES_BLOCK_SIZE;
  
  for(uint32_t* wp = (uint32_t*)(data + size - AES_BLOCK_SIZE); size > 0; wp -= AES_BLOCK_WSIZE, size -= AES_BLOCK_SIZE, out_addr -= AES_BLOCK_WSIZE) {
    // copy block & decrypt in place
    memcpy(state, wp + 4, AES_BLOCK_SIZE);
    aes128_dec_block(state, rkeys);
    
    // XOR with previously encrypted block
    wstate[0] ^= wp[0];
    wstate[1] ^= wp[1];
    wstate[2] ^= wp[2];
    wstate[3] ^= wp[3];
    
    AES_SAVE_STATE(out_addr, wstate, memtype); 
  }
  
  memcpy(state, data, AES_BLOCK_SIZE);
  aes128_dec_block(state, rkeys);
  
  // XOR 1st block with init vector
  uint32_t* wiv = (uint32_t*)iv;
  wstate[0] ^= wiv[0];
  wstate[1] ^= wiv[1];
  wstate[2] ^= wiv[2];
  wstate[3] ^= wiv[3];
  
  AES_SAVE_STATE(out_addr, wstate, memtype); 
}
